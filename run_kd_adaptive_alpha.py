#!/usr/bin/env python3
# Experiment B: Loss-ratio adaptive α (no smoothing)
...
# ---------- KD Training ----------
def train_kd(student, dataloader, optimizer, kd_criterion, gt_criterion, device):
    student.train()
    total_loss = 0.0
    for xb, y_teacher, y_true in dataloader:
        ...
        with torch.no_grad():
            alpha = L_true / (L_true + L_teacher + 1e-8)
...
def main():
    ...
    save_path = os.path.join(args.save_dir, f"{'_'.join(args.datasets)}_student_adaptive_alpha.pth")
    torch.save(student.state_dict(), save_path)
    print(f"✅ Student model (Exp B) saved to {save_path}")
