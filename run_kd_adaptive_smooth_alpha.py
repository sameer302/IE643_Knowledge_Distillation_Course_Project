...
# ---------- KD Training ----------
def train_kd(student, dataloader, optimizer, kd_criterion, gt_criterion, device, smooth_beta=0.9):
    """
    Adaptive Knowledge Distillation training loop.
    α is computed dynamically per batch as:
        α = L_true / (L_true + L_teacher)
    and smoothed over time to avoid oscillations.
    """
    student.train()
    total_loss = 0.0
    prev_alpha = 0.5  # initialize with neutral trust (equal teacher & ground truth)
    ...
    for xb, y_teacher, y_true in dataloader:
        ...
        # ---- adaptive α (loss ratio) ----
        with torch.no_grad():
            alpha_inst = L_true / (L_true + L_teacher + 1e-8)
            alpha = smooth_beta * prev_alpha + (1 - smooth_beta) * alpha_inst
            prev_alpha = alpha.detach()
    ...
    print(f"   ↳ Avg KD loss: {avg_loss:.6f} | Last α: {alpha.item():.3f}")
    return avg_loss
...
def main():
    ...
    # Move all data tensors to the selected device (important!)
    X = X.to(device)
    ...
    for epoch in range(args.epochs):
        loss = train_kd(student, loader, optimizer, kd_criterion, gt_criterion, device)
        print(f"Epoch {epoch+1}/{args.epochs} | KD Loss: {loss:.6f}")

    save_name = f"{'_'.join(args.datasets)}_student_projected_adaptive_smoothing.pth"
